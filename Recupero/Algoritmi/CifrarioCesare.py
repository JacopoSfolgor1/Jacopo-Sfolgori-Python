...
def caesar_cypher_encrypt(secret: str, key: int) -> str:
    cypher = ""
    secret = secret.strip().lower()

    for char in secret:
        if char in ces:
            index = ces.index(char)
            new_index = (index + key) % 26
            cypher += ces[new_index]
        else:
            cypher += char  
    return cypher

def caesar_cypher_decrypt(secret: str, key: int) -> str:
    cypher = ""
    secret = secret.strip().lower()
    for char in secret:
        if char in ces:
...
print(caesar_cypher_decrypt("ciao", 2))
